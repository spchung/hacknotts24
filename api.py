...
@app.get("/get_by_doc_id/{doc_id}")
async def get_by_doc_id(doc_id):
  return {
    'status': 200,
    'message': Graph.get_by_doc_id(doc_id)
  }

@app.get("/get_by_term_id/{term_id}")
async def get_by_term_id(term_id):
  return {
    'status': 200,
    'message': Graph.get_terms_by_doc(term_id)
  }